                    ...
                    <div class="tab-content">
                        <form action="admin/pengaduans/{{ $pengaduan->slug }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <select class="form-select" name="perihal_id">
                                            @foreach ($perihals as $perihal)
                                                @if (old('perihal_id', $pengaduan->perihal_id) == $perihal->id)
                                                    <option value="{{ $perihal->id }}" selected>{{ $perihal->name }}
                                                    </option>
                                                @else
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="mb-3">
                                        ...
                                        <input type="text"
                                            class="form-control text-capitalize @error('subjek') is-invalid @enderror"
                            ...
                            <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug"
                                id="slug" style="opacity: 0; height: 1px; display: none" placeholder="Masukkan slug"
                                value="{{ old('slug', $pengaduan->slug) }}" required />
                            @error('slug')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                            ...
                            <div class="text-end">
                                <button type="submit" class="btn btn-success mt-2"><i class="bx bxs-send"></i> Edit
                                    Pengaduan</button>
                            </div>
                        </form>
                    </div>
